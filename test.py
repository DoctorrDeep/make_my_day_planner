...
for a_timeMin, a_free_result_list in timeMins_raw.items():
    ...
    for i in a_free_result_list:
        temp_free_timeblocks.append([f"{time_s_p[0]}{i[0]}{time_s_p[1]}", f"{time_s_p[0]}{i[1]}{time_s_p[1]}"])

    timeMin_free_time_dict[f"{time_s_p[0]}{a_timeMin}{time_s_p[1]}"] = temp_free_timeblocks
...
for timeMin, expected_free_timeblocks in timeMin_free_time_dict.items():
    ...
    free_timeblock_expected_and_received_results.append([expected_free_timeblocks, received_free_timeblocks])
...
@pytest.mark.parametrize('expected_free_timeblocks, received_free_timeblocks', free_timeblock_expected_and_received_results)
